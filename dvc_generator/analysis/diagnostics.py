...
Usage:
    python -m dvc_generator.analysis.diagnostics --tif_dir data_tif/your_experiment
"""
...
def analyze_tif_file(tif_path):
    ...
    # Load — if tifffile mis-reads OME metadata as 2D, retry without OME parsing
    vol = tifffile.imread(tif_path)
    if vol.ndim == 2:
        vol = tifffile.imread(tif_path, is_ome=False)
    ...
    print(f"\nSampling 5 random 128^3 regions:")
    if vol_norm.ndim != 3:
        print(f"  (Skipped — volume is {vol_norm.ndim}D, expected 3D)")
        return vol_norm
    ...
    if D >= 128 and H >= 128 and W >= 128:
        ...
        print("  (Volume too small to sample 128^3 regions)")
